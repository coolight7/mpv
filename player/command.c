...
static const struct m_option udata_type = {.type = CONF_TYPE_NODE};

static const struct m_option mdata_type = {.type = CONF_TYPE_NODE};
...
static int edit_filters(struct MPContext *mpctx, struct mp_log *log,
                        enum stream_type mediatype, const char *cmd,
                        const char *arg);
...
static int invoke_hook_handler(struct MPContext *mpctx,
                               struct hook_handler *h) {
...
static int run_next_hook_handler(struct MPContext *mpctx, char *type,
                                 int start) {
  struct command_ctx *cmd = mpctx->command_ctx;
...
static char *append_selected_style(struct MPContext *mpctx, char *str) {
  ...
  return talloc_asprintf_append(
      str,
      "%s{\\b1\\1c&H%02hhx%02hhx%02hhx&\\1a&H%02hhx&\\3c&H%02hhx%02hhx%02hhx&"
      "\\3a&H%02hhx&}%s",
      OSD_ASS_0, mpctx->video_out->osd->opts->osd_selected_color.b,
      ...
      (uint8_t)(255 -
                mpctx->video_out->osd->opts->osd_selected_outline_color.a),
      OSD_ASS_1);
}

static const char *get_style_reset(struct MPContext *mpctx) {
  return mpctx->video_out && mpctx->opts->video_osd ? OSD_ASS_0
             "{\\r}" OSD_ASS_1
...
static char *cut_osd_list(struct MPContext *mpctx, char *header, char *text,
                          int pos) {
  ...
  if (mpctx->video_out && mpctx->opts->video_osd) {
    ...
    char *msg =
        mp_property_expand_escaped_string(mpctx, mpctx->opts->status_msg);
  ...
  new = talloc_asprintf_append_buffer(
      new, "%.*s", (int)(tail ? tail - head : strlen(head)), head);
...
static int mp_property_playback_speed(void *ctx, struct m_property *prop,
  ...
  if (action == M_PROPERTY_PRINT || action == M_PROPERTY_FIXED_LEN_PRINT) {
    *(char **)arg =
        mp_format_double(NULL, mpctx->opts->playback_speed, 2, false, false,
                         action != M_PROPERTY_FIXED_LEN_PRINT);
    return M_PROPERTY_OK;
  }
...
static int mp_property_playback_pitch(void *ctx, struct m_property *prop,
  ...
  if (action == M_PROPERTY_PRINT || action == M_PROPERTY_FIXED_LEN_PRINT) {
    *(char **)arg =
        mp_format_double(NULL, mpctx->opts->playback_pitch, 2, false, false,
                         action != M_PROPERTY_FIXED_LEN_PRINT);
    return M_PROPERTY_OK;
  }
  return mp_property_generic_option(mpctx, prop, action, arg);
...
static int mp_property_av_speed_correction(void *ctx, struct m_property *prop,
  ...
  switch (type[0]) {
  case 'a':
    val = mpctx->speed_factor_a;
    break;
  case 'v':
    val = mpctx->speed_factor_v;
    break;
  default:
    MP_ASSERT_UNREACHABLE();
  }
  ...
  if (action == M_PROPERTY_PRINT || action == M_PROPERTY_FIXED_LEN_PRINT) {
    *(char **)arg = mp_format_double(NULL, (val - 1) * 100, 2, true, true,
                                     action != M_PROPERTY_FIXED_LEN_PRINT);
    return M_PROPERTY_OK;
  }
...
static int mp_property_pid(void *ctx, struct m_property *prop, int action,
                           void *arg) {
...
static int mp_property_path(void *ctx, struct m_property *prop, int action,
                            void *arg) {
  MPContext *mpctx = ctx;
  if (!mpctx->filename)
...
static int mp_property_filename(void *ctx, struct m_property *prop, int action,
                                void *arg) {
  MPContext *mpctx = ctx;
  if (!mpctx->filename)
    return M_PROPERTY_UNAVAILABLE;
...
static int mp_property_file_size(void *ctx, struct m_property *prop, int action,
                                 void *arg) {
  MPContext *mpctx = ctx;
  if (!mpctx->demuxer)
    return M_PROPERTY_UNAVAILABLE;
...
/// Demuxer name (RO)
static int mp_property_demuxer(void *ctx, struct m_property *prop, int action,
                               void *arg) {
  MPContext *mpctx = ctx;
...
static int mp_property_file_format(void *ctx, struct m_property *prop,
  ...
  if (!demuxer)
    return M_PROPERTY_UNAVAILABLE;
  const char *name =
      demuxer->filetype ? demuxer->filetype : demuxer->desc->name;
  return m_property_strdup_ro(action, arg, name);
}
...
static int mp_property_duration(void *ctx, struct m_property *prop, int action,
                                void *arg) {
  MPContext *mpctx = ctx;
...
static int mp_property_avsync(void *ctx, struct m_property *prop, int action,
                              void *arg) {
  MPContext *mpctx = ctx;
  ...
  if (action == M_PROPERTY_PRINT || action == M_PROPERTY_FIXED_LEN_PRINT) {
    *(char **)arg =
        mp_format_double(NULL, mpctx->last_av_difference, 4, true, false,
                         action != M_PROPERTY_FIXED_LEN_PRINT);
    return M_PROPERTY_OK;
  }
...
static int mp_property_vo_delayed_frame_count(void *ctx,
...
static int mp_property_time_pos(void *ctx, struct m_property *prop, int action,
                                void *arg) {
  MPContext *mpctx = ctx;
  if (!mpctx->playback_initialized)
    return M_PROPERTY_UNAVAILABLE;
...
static int mp_property_audio_pts(void *ctx, struct m_property *prop, int action,
                                 void *arg) {
  MPContext *mpctx = ctx;
...
static int mp_property_remaining(void *ctx, struct m_property *prop, int action,
                                 void *arg) {
  double remaining;
...
/// Current chapter (RW)
static int mp_property_chapter(void *ctx, struct m_property *prop, int action,
                               void *arg) {
  MPContext *mpctx = ctx;
  if (!mpctx->playback_initialized)
    return M_PROPERTY_UNAVAILABLE;
  ...
  case M_PROPERTY_SET:;
    ...
    if (action == M_PROPERTY_SWITCH) {
      ...
      if (mpctx->opts->chapter_seek_threshold >= 0 && step_all < 0) {
        double current_chapter_start = chapter_start_time(mpctx, chapter);
...
static int mp_property_edition(void *ctx, struct m_property *prop, int action,
                               void *arg) {
  MPContext *mpctx = ctx;
  struct demuxer *demuxer = mpctx->demuxer;
  char *name = NULL;
...
static int get_edition_entry(int item, int action, void *arg, void *ctx) {
  ...
  struct m_sub_property props[] = {
      ...
      {"title", SUB_PROP_STR(title), .unavailable = !title},
...
static int mp_property_chapters(void *ctx, struct m_property *prop, int action,
                                void *arg) {
  MPContext *mpctx = ctx;
  if (!mpctx->playback_initialized)
    return M_PROPERTY_UNAVAILABLE;
...
static int mp_property_editions(void *ctx, struct m_property *prop, int action,
                                void *arg) {
  MPContext *mpctx = ctx;
  struct demuxer *demuxer = mpctx->demuxer;
  if (!demuxer)
...
static int get_tag_entry(int item, int action, void *arg, void *ctx) {
  ...
  struct m_sub_property props[] = {{"key", SUB_PROP_STR(tags->keys[item])},
...
static int tag_property(int action, void *arg, struct mp_tags *tags) {
  ...
  case M_PROPERTY_KEY_ACTION: {
    ...
    if (bstr_equals0(key, "list")) {
      ...
      nka.key = rem;
      return m_property_read_list(action, &nka, tags->num_keys, get_tag_entry,
                                  tags);
    }
...
/// Demuxer meta data
static int mp_property_metadata(void *ctx, struct m_property *prop, int action,
                                void *arg) {
  MPContext *mpctx = ctx;
  struct demuxer *demuxer = mpctx->demuxer;
  if (!demuxer)
...
static int mp_property_core_idle(void *ctx, struct m_property *prop, int action,
                                 void *arg) {
  MPContext *mpctx = ctx;
...
static int mp_property_idle(void *ctx, struct m_property *prop, int action,
                            void *arg) {
  MPContext *mpctx = ctx;
  return m_property_bool_ro(action, arg, mpctx->stop_play == PT_STOP);
}

static int mp_property_window_id(void *ctx, struct m_property *prop, int action,
                                 void *arg) {
  MPContext *mpctx = ctx;
...
static int mp_property_eof_reached(void *ctx, struct m_property *prop,
  ...
  if (!mpctx->playback_initialized)
    return M_PROPERTY_UNAVAILABLE;
  bool eof =
      mpctx->video_status == STATUS_EOF && mpctx->audio_status == STATUS_EOF;
  return m_property_bool_ro(action, arg, eof);
}

static int mp_property_seeking(void *ctx, struct m_property *prop, int action,
                               void *arg) {
  MPContext *mpctx = ctx;
  if (!mpctx->playback_initialized)
    return M_PROPERTY_UNAVAILABLE;
...
static int mp_property_demuxer_cache_duration(void *ctx,
                                              struct m_property *prop,
                                              int action, void *arg) {
  MPContext *mpctx = ctx;
  if (!mpctx->demuxer)
...
static int mp_property_clock(void *ctx, struct m_property *prop, int action,
                             void *arg) {
  char outstr[6];
  time_t t = time(NULL);
...
static int mp_property_seekable(void *ctx, struct m_property *prop, int action,
                                void *arg) {
  MPContext *mpctx = ctx;
  if (!mpctx->demuxer)
    return M_PROPERTY_UNAVAILABLE;
...
/// Volume (RW)
static int mp_property_volume(void *ctx, struct m_property *prop, int action,
                              void *arg) {
  MPContext *mpctx = ctx;
...
static int mp_property_ao_volume(void *ctx, struct m_property *prop, int action,
                                 void *arg) {
  MPContext *mpctx = ctx;
...
static int mp_property_ao_mute(void *ctx, struct m_property *prop, int action,
                               void *arg) {
  MPContext *mpctx = ctx;
  struct ao *ao = mpctx->ao;
  if (!ao)
...
static int get_device_entry(int item, int action, void *arg, void *ctx) {
  ...
  struct m_sub_property props[] = {{"name", SUB_PROP_STR(entry->name)},
...
static void create_hotplug(struct MPContext *mpctx) {
  ...
  if (!cmd->hotplug) {
    cmd->hotplug = ao_hotplug_create(mpctx->global, mp_wakeup_core_cb, mpctx);
  }
}
...
static int mp_property_audio_device(void *ctx, struct m_property *prop,
  ...
  if (action == M_PROPERTY_PRINT) {
    ...
    struct ao_device_list *list =
        ao_hotplug_get_device_list(cmd->hotplug, mpctx->ao);
...
static int mp_property_audio_devices(void *ctx, struct m_property *prop,
  ...
  struct ao_device_list *list =
      ao_hotplug_get_device_list(cmd->hotplug, mpctx->ao);
  return m_property_read_list(action, arg, list->num_devices, get_device_entry,
                              list);
}

static int mp_property_ao(void *ctx, struct m_property *p, int action,
                          void *arg) {
  MPContext *mpctx = ctx;
...
static int mp_property_audio_fft(void *ctx, struct m_property *prop, int action,
                                 void *arg) {
  MPContext *mpctx = ctx;
  ...
  case M_PROPERTY_GET_NODE: {
    if (!mpctx->audio_fft_output_valid || !mpctx->audio_fft_output) {
    ...
    node->u.ba = talloc(NULL, mpv_byte_array);
    node->u.ba->data = mpctx->audio_fft_output;
    node->u.ba->size = mpctx->audio_fft_output_size;
    return M_PROPERTY_OK;
  }
  }
  return M_PROPERTY_NOT_IMPLEMENTED;
...
static int mp_property_audio_params(void *ctx, struct m_property *prop,
                                    int action, void *arg) {
  MPContext *mpctx = ctx;
  return property_audiofmt(
      mpctx->ao_chain ? mpctx->ao_chain->filter->input_aformat : NULL, action,
      arg);
}
...
static int mp_property_switch_track(void *ctx, struct m_property *prop,
  ...
  case M_PROPERTY_PRINT:
    if (track) {
      ...
      *(char **)arg =
          talloc_asprintf(NULL, "(%d) %s", track->user_tid,
...
static int get_track_entry(int item, int action, void *arg, void *ctx) {
  ...
  struct mp_tags *tags =
      track->stream ? track->stream->tags : &(struct mp_tags){0};
  ...
  struct m_sub_property props[] = {
      ...
      {"title", SUB_PROP_STR(track->title), .unavailable = !track->title},
      {"lang", SUB_PROP_STR(track->lang), .unavailable = !track->lang},
      ...
      {"decoder", SUB_PROP_STR(p.decoder), .unavailable = !p.decoder},
      {"decoder-desc", SUB_PROP_STR(p.decoder_desc),
       .unavailable = !p.decoder_desc},
      {"codec", SUB_PROP_STR(p.codec), .unavailable = !p.codec},
      {"codec-desc", SUB_PROP_STR(p.codec_desc), .unavailable = !p.codec_desc},
      ...
      {"demux-crop-w", SUB_PROP_INT(mp_rect_w(p.crop)),
       .unavailable = !has_crop},
      {"demux-crop-h", SUB_PROP_INT(mp_rect_h(p.crop)),
       .unavailable = !has_crop},
      ...
      {"format-name", SUB_PROP_STR(p.format_name),
       .unavailable = !p.format_name},
      ...
      {"dolby-vision-profile", SUB_PROP_INT(p.dv_profile),
       .unavailable = !p.dovi},
      {"dolby-vision-level", SUB_PROP_INT(p.dv_level), .unavailable = !p.dovi},
...
static int mp_property_list_tracks(void *ctx, struct m_property *prop,
  ...
  if (action == M_PROPERTY_PRINT) {
    ...
    if (demuxer && demuxer->num_editions > 1) {
      res = talloc_asprintf_append(res, "\nEdition: %d of %d",
                                   demuxer->edition + 1, demuxer->num_editions);
    } else {
      res[strlen(res) - 1] = '\0';
    }
  ...
  return m_property_read_list(action, arg, mpctx->num_tracks, get_track_entry,
                              mpctx);
}
...
static int property_imgparams(const struct mp_image_params *p, int action,
                              void *arg) {
  ...
  if (fmt) {
    ...
    if (!!(desc.flags & MP_IMGFLAG_ALPHA) != (alpha != PL_ALPHA_UNKNOWN))
      alpha = (desc.flags & MP_IMGFLAG_ALPHA) ? PL_ALPHA_INDEPENDENT
                                              : PL_ALPHA_UNKNOWN;
#endif
  }
  ...
  bool custom_prim = pl_primaries_valid(&hdr->prim) &&
                     !pl_raw_primaries_similar(
                         &hdr->prim, pl_raw_primaries_get(p->color.primaries));

  bool has_crop = mp_rect_w(p->crop) > 0 && mp_rect_h(p->crop) > 0;
  const char *aspect_name = get_aspect_ratio_name(d_w / (double)d_h);
  const char *sar_name = get_aspect_ratio_name(p->w / (double)p->h);
  const char *pixelformat_name =
      p->imgfmt_name ? p->imgfmt_name : mp_imgfmt_to_name(p->imgfmt);
  struct m_sub_property props[] = {
      ...
      {"average-bpp", SUB_PROP_INT(bpp), .unavailable = !bpp},
      {"w", SUB_PROP_INT(p->w)},
      {"h", SUB_PROP_INT(p->h)},
      {"dw", SUB_PROP_INT(d_w)},
      {"dh", SUB_PROP_INT(d_h)},
      ...
      {"light", SUB_PROP_STR(m_opt_choice_str(mp_csp_light_names, p->light))},
      {"chroma-location",
      ...
      {"alpha", SUB_PROP_STR(m_opt_choice_str(pl_alpha_names, alpha)),
      ...
      {"scene-max-r", SUB_PROP_FLOAT(hdr->scene_max[0]),
       .unavailable = !has_hdr10plus},
      {"scene-max-g", SUB_PROP_FLOAT(hdr->scene_max[1]),
       .unavailable = !has_hdr10plus},
      {"scene-max-b", SUB_PROP_FLOAT(hdr->scene_max[2]),
       .unavailable = !has_hdr10plus},
      {"scene-avg", SUB_PROP_FLOAT(hdr->scene_avg),
       .unavailable = !has_hdr10plus},
      {"max-pq-y", SUB_PROP_FLOAT(hdr->max_pq_y), .unavailable = !has_cie_y},
      {"avg-pq-y", SUB_PROP_FLOAT(hdr->avg_pq_y), .unavailable = !has_cie_y},
      {"prim-red-x", SUB_PROP_FLOAT(hdr->prim.red.x),
       .unavailable = !custom_prim},
      {"prim-red-y", SUB_PROP_FLOAT(hdr->prim.red.y),
       .unavailable = !custom_prim},
      {"prim-green-x", SUB_PROP_FLOAT(hdr->prim.green.x),
       .unavailable = !custom_prim},
      {"prim-green-y", SUB_PROP_FLOAT(hdr->prim.green.y),
       .unavailable = !custom_prim},
      {"prim-blue-x", SUB_PROP_FLOAT(hdr->prim.blue.x),
       .unavailable = !custom_prim},
      {"prim-blue-y", SUB_PROP_FLOAT(hdr->prim.blue.y),
       .unavailable = !custom_prim},
      {"prim-white-x", SUB_PROP_FLOAT(hdr->prim.white.x),
       .unavailable = !custom_prim},
      {"prim-white-y", SUB_PROP_FLOAT(hdr->prim.white.y),
       .unavailable = !custom_prim},
      {0}};
...
static int mp_property_vd_imgparams(void *ctx, struct m_property *prop,
  ...
  if (vo_c->filter->input_params.imgfmt) {
    ...
    struct m_sub_property props[] = {
        {"w", SUB_PROP_INT(c->disp_w)}, {"h", SUB_PROP_INT(c->disp_h)}, {0}};
    return m_property_read_sub(props, action, arg);
  }
  return M_PROPERTY_UNAVAILABLE;
}
...
static int mp_property_video_frame_info(void *ctx, struct m_property *prop,
  ...
  const char *pict_type =
      f->pict_type >= 1 && f->pict_type <= 3 ? pict_types[f->pict_type] : NULL;
  ...
  for (int n = 0; n < f->num_ff_side_data; n++) {
    ...
    if (mpctx->vo_chain && mpsd->type == AV_FRAME_DATA_S12M_TIMECODE) {
      av_timecode_make_smpte_tc_string2(
          s12m_tc, av_d2q(mpctx->vo_chain->filter->container_fps, INT_MAX),
  ...
  struct m_sub_property props[] = {
      ...
      {"smpte-timecode", SUB_PROP_STR(s12m_tc),
       .unavailable = s12m_tc[0] == '\0'},
      {"estimated-smpte-timecode", SUB_PROP_STR(approx_smpte),
       .unavailable = approx_smpte[0] == '\0'},
      {0}};
...
static int mp_property_current_window_scale(void *ctx, struct m_property *prop,
  ...
  int s[2];
  if (vo_control(vo, VOCTRL_GET_UNFS_WINDOW_SIZE, s) <= 0 || s[0] < 1 ||
      s[1] < 1)
    return M_PROPERTY_UNAVAILABLE;
...
static void update_hidpi_window_scale(struct MPContext *mpctx,
                                      bool hidpi_scale) {
  struct command_ctx *cmd = mpctx->command_ctx;
  struct vo *vo = mpctx->video_out;
  ...
  double scale =
      hidpi_scale ? cmd->cached_window_scale : 1 / cmd->cached_window_scale;
...
static int mp_property_focused(void *ctx, struct m_property *prop, int action,
                               void *arg) {
  MPContext *mpctx = ctx;
  struct vo *vo = mpctx->video_out;
  if (!vo)
...
static void get_frame_perf(struct mpv_node *node, struct mp_frame_perf *perf) {
  for (int i = 0; i < perf->count; i++) {
    ...
    struct mpv_node *samples =
        node_map_add(pass, "samples", MPV_FORMAT_NODE_ARRAY);
...
static char *asprint_perf(char *res, struct mp_frame_perf *perf) {
  for (int i = 0; i < perf->count; i++) {
    ...
    res = talloc_asprintf_append(res, "- %s: last %dus avg %dus peak %dus\n",
                                 perf->desc[i], (int)pass->last / 1000,
                                 (int)pass->avg / 1000, (int)pass->peak / 1000);
  }

  return res;
}

static int mp_property_vo_passes(void *ctx, struct m_property *prop, int action,
                                 void *arg) {
  MPContext *mpctx = ctx;
  if (!mpctx->video_out)
    return M_PROPERTY_UNAVAILABLE;
  ...
  case M_PROPERTY_GET: {
    ...
    struct mpv_node *fresh =
        node_map_add(&node, "fresh", MPV_FORMAT_NODE_ARRAY);
    struct mpv_node *redraw =
        node_map_add(&node, "redraw", MPV_FORMAT_NODE_ARRAY);
...
static int mp_property_vo(void *ctx, struct m_property *p, int action,
                          void *arg) {
  MPContext *mpctx = ctx;
  return m_property_strdup_ro(
      action, arg, mpctx->video_out ? mpctx->video_out->driver->name : NULL);
}

static int mp_property_gpu_context(void *ctx, struct m_property *p, int action,
                                   void *arg) {
  MPContext *mpctx = ctx;
  return m_property_strdup_ro(
      action, arg, mpctx->video_out ? mpctx->video_out->context_name : NULL);
}

static int mp_property_osd_dim(void *ctx, struct m_property *prop, int action,
                               void *arg) {
  MPContext *mpctx = ctx;
  ...
  struct m_sub_property props[] = {{"w", SUB_PROP_INT(vo_res.w)},
...
static int mp_property_osd_sym(void *ctx, struct m_property *prop, int action,
                               void *arg) {
  MPContext *mpctx = ctx;
  char temp[20];
...
static int mp_property_osd_ass(void *ctx, struct m_property *prop, int action,
                               void *arg) {
  struct m_sub_property props[] = {
      {"0", SUB_PROP_STR(OSD_ASS_0)}, {"1", SUB_PROP_STR(OSD_ASS_1)}, {0}};
  return m_property_read_sub(props, action, arg);
}

static int mp_property_term_clip(void *ctx, struct m_property *prop, int action,
                                 void *arg) {
  return m_property_strdup_ro(action, arg, TERM_MSG_0);
}

static int mp_property_term_size(void *ctx, struct m_property *prop, int action,
                                 void *arg) {
  int w = -1, h = -1;
  ...
  struct m_sub_property props[] = {
      {"w", SUB_PROP_INT(w)}, {"h", SUB_PROP_INT(h)}, {0}};

  return m_property_read_sub(props, action, arg);
}

static int mp_property_mouse_pos(void *ctx, struct m_property *prop, int action,
                                 void *arg) {
  MPContext *mpctx = ctx;
...
static int get_touch_pos(int item, int action, void *arg, void *ctx) {
  ...
  struct m_sub_property props[] = {{"x", SUB_PROP_INT(pos[0][item])},
...
static int mp_property_touch_pos(void *ctx, struct m_property *prop, int action,
                                 void *arg) {
  MPContext *mpctx = ctx;
  int xs[MAX_TOUCH_POINTS], ys[MAX_TOUCH_POINTS], ids[MAX_TOUCH_POINTS];
  int count =
      mp_input_get_touch_pos(mpctx->input, MAX_TOUCH_POINTS, xs, ys, ids);
...
static int mp_property_tablet_pos(void *ctx, struct m_property *prop,
  ...
  case M_PROPERTY_GET: {
    ...
    mp_input_get_tablet_pos(mpctx->input, &xs, &ys, &tool_in_proximity,
                            &tool_down, &tool_stylus_btn1_pressed,
                            &tool_stylus_btn2_pressed,
                            &tool_stylus_btn3_pressed, &pad_focus,
                            &pad_buttons_pressed, &pad_buttons);
    ...
    node_map_add_string(&node, "tool-stylus-btn1",
                        tool_stylus_btn1_pressed ? "pressed" : "released");
    node_map_add_string(&node, "tool-stylus-btn2",
                        tool_stylus_btn2_pressed ? "pressed" : "released");
    node_map_add_string(&node, "tool-stylus-btn3",
                        tool_stylus_btn3_pressed ? "pressed" : "released");
    node_map_add_flag(&node, "pad-focus", pad_focus);

    struct mpv_node *args =
        node_map_add(&node, "pad-btns", MPV_FORMAT_NODE_MAP);
    for (int i = 0; i < pad_buttons; i++) {
      char *name = mp_tprintf(2, "%d", i + 1);
      node_map_add_string(args, name,
                          pad_buttons_pressed[i] ? "pressed" : "released");
    }
...
/// Video fps (RO)
static int mp_property_fps(void *ctx, struct m_property *prop, int action,
                           void *arg) {
  MPContext *mpctx = ctx;
...
static int mp_property_vf_fps(void *ctx, struct m_property *prop, int action,
                              void *arg) {
  MPContext *mpctx = ctx;
  if (!mpctx->vo_chain)
    return M_PROPERTY_UNAVAILABLE;
...
/// Subtitle delay (RW)
static int mp_property_sub_delay(void *ctx, struct m_property *prop, int action,
                                 void *arg) {
  MPContext *mpctx = ctx;
  struct MPOpts *opts = mpctx->opts;
...
/// Subtitle speed (RW)
static int mp_property_sub_speed(void *ctx, struct m_property *prop, int action,
                                 void *arg) {
  MPContext *mpctx = ctx;
  struct MPOpts *opts = mpctx->opts;
  if (action == M_PROPERTY_PRINT) {
...
static int mp_property_sub_pos(void *ctx, struct m_property *prop, int action,
                               void *arg) {
  MPContext *mpctx = ctx;
  struct MPOpts *opts = mpctx->opts;
  int track_ind = *(int *)prop->priv;
  if (action == M_PROPERTY_PRINT || action == M_PROPERTY_FIXED_LEN_PRINT) {
    *(char **)arg =
        mp_format_double(NULL, opts->subs_shared->sub_pos[track_ind], 2, false,
...
static int mp_property_sub_text(void *ctx, struct m_property *prop, int action,
                                void *arg) {
  MPContext *mpctx = ctx;
  const int *def = prop->priv;
  int sub_index = def[0];
...
static struct sd_times get_times(void *ctx, struct m_property *prop, int action,
                                 void *arg) {
...
static int mp_property_sub_start(void *ctx, struct m_property *prop, int action,
                                 void *arg) {
...
static int mp_property_sub_end(void *ctx, struct m_property *prop, int action,
                               void *arg) {
...
static int get_playlist_entry(int item, int action, void *arg, void *ctx) {
  ...
  struct m_sub_property props[] = {
      ...
      {"playlist-path", SUB_PROP_STR(e->playlist_path),
       .unavailable = !e->playlist_path},
      {0}};
...
static int mp_property_playlist(void *ctx, struct m_property *prop, int action,
                                void *arg) {
  MPContext *mpctx = ctx;
  if (action == M_PROPERTY_PRINT) {
    struct playlist *pl = mpctx->playlist;
  ...
  return m_property_read_list(action, arg,
                              playlist_entry_count(mpctx->playlist),
...
static int mp_property_vf(void *ctx, struct m_property *prop, int action,
                          void *arg) {
  return property_filter(prop, action, arg, ctx, STREAM_VIDEO);
}

static int mp_property_af(void *ctx, struct m_property *prop, int action,
                          void *arg) {
  return property_filter(prop, action, arg, ctx, STREAM_AUDIO);
}

static int mp_property_ab_loop(void *ctx, struct m_property *prop, int action,
                               void *arg) {
  struct MPContext *mpctx = ctx;
  if (action == M_PROPERTY_KEY_ACTION) {
    double val;
...
static int mp_property_cwd(void *ctx, struct m_property *prop, int action,
                           void *arg) {
  switch (action) {
  case M_PROPERTY_GET: {
...
static int mp_property_current_watch_later_dir(void *ctx,
                                               struct m_property *prop,
                                               int action, void *arg) {
  MPContext *mpctx = ctx;
...
static int mp_property_protocols(void *ctx, struct m_property *prop, int action,
                                 void *arg) {
  switch (action) {
  case M_PROPERTY_GET:
...
static int mp_property_keylist(void *ctx, struct m_property *prop, int action,
                               void *arg) {
  switch (action) {
  case M_PROPERTY_GET:
...
static int get_decoder_entry(int item, int action, void *arg, void *ctx) {
  ...
  struct m_sub_property props[] = {{"codec", SUB_PROP_STR(c->codec)},
...
static int mp_property_decoders(void *ctx, struct m_property *prop, int action,
                                void *arg) {
...
static int mp_property_encoders(void *ctx, struct m_property *prop, int action,
                                void *arg) {
  struct mp_decoder_list *codecs = talloc_zero(NULL, struct mp_decoder_list);
...
static int mp_property_version(void *ctx, struct m_property *prop, int action,
                               void *arg) {
...
static int mp_property_ffmpeg(void *ctx, struct m_property *prop, int action,
                              void *arg) {
...
static int mp_property_platform(void *ctx, struct m_property *prop, int action,
                                void *arg) {
  return m_property_strdup_ro(action, arg, PLATFORM);
}

static int mp_property_alias(void *ctx, struct m_property *prop, int action,
                             void *arg) {
...
static int mp_property_deprecated_alias(void *ctx, struct m_property *prop,
  ...
  MP_WARN(mpctx,
          "Warning: property '%s' was replaced with '%s' and "
...
static int access_options(struct m_property_action_arg *ka, bool local,
                          MPContext *mpctx) {
  struct m_config_option *opt = m_config_get_co(mpctx->mconfig, bstr0(ka->key));
  if (!opt)
    return M_PROPERTY_UNKNOWN;
  if (!opt->data)
    return M_PROPERTY_UNAVAILABLE;
...
static int access_option_list(int action, void *arg, bool local,
                              MPContext *mpctx) {
  switch (action) {
  case M_PROPERTY_GET_TYPE:
    *(struct m_option *)arg = (struct m_option){.type = CONF_TYPE_STRING_LIST};
...
static int mp_property_options(void *ctx, struct m_property *prop, int action,
                               void *arg) {
  MPContext *mpctx = ctx;
...
static int mp_property_option_info(void *ctx, struct m_property *prop,
  ...
  case M_PROPERTY_KEY_ACTION: {
    ...
    bool has_minmax =
        opt->min < opt->max && (opt->type->flags & M_OPT_TYPE_USES_RANGE);
    char **choices = NULL;
...
static int mp_property_list(void *ctx, struct m_property *prop, int action,
                            void *arg) {
  struct MPContext *mpctx = ctx;
  struct command_ctx *cmd = mpctx->command_ctx;
...
static int mp_profile_list(void *ctx, struct m_property *prop, int action,
                           void *arg) {
  MPContext *mpctx = ctx;
  switch (action) {
  case M_PROPERTY_GET_TYPE:
...
static int mp_property_commands(void *ctx, struct m_property *prop, int action,
                                void *arg) {
  switch (action) {
  case M_PROPERTY_GET_TYPE:
    *(struct m_option *)arg = (struct m_option){.type = CONF_TYPE_NODE};
...
static int mp_property_bindings(void *ctx, struct m_property *prop, int action,
                                void *arg) {
  MPContext *mpctx = ctx;
  switch (action) {
  case M_PROPERTY_GET_TYPE:
...
static int mp_property_mdata(void *ctx, struct m_property *prop, int action,
                             void *arg) {
  MPContext *mpctx = ctx;
...
static int do_op_udata(struct udata_ctx *ctx, int action, void *arg) {
  ...
  case M_PROPERTY_KEY_ACTION: {
    ...
    // If we're operating on an array, sub-object access is handled by
    // m_property_read_list
    if (node->format == MPV_FORMAT_NODE_ARRAY)
      return m_property_read_list(action, arg, node->u.list->num,
                                  &do_list_udata, ctx);
    ...
    if (!cnode) {
      ...
      case M_PROPERTY_SET_NODE: {
        ...
        // otherwise, make it NONE, since we'll be overwriting it at the next
        // level.
        cnode = node_map_badd(
            node, key, has_split ? MPV_FORMAT_NODE_MAP : MPV_FORMAT_NONE);
        if (!cnode)
          return M_PROPERTY_ERROR;
        break;
      case M_PROPERTY_GET_TYPE:
...
static int mp_property_udata(void *ctx, struct m_property *prop, int action,
                             void *arg) {
...
static int mp_property_current_clipboard_backend(void *ctx,
                                                 struct m_property *p,
                                                 int action, void *arg) {
  MPContext *mpctx = ctx;
  return m_property_strdup_ro(action, arg,
                              mp_clipboard_get_backend_name(mpctx->clipboard));
}
...
static int get_clipboard(struct MPContext *mpctx, void *arg,
  ...
  if (ret != CLIPBOARD_SUCCESS) {
    talloc_free(tmp);
    return ret == CLIPBOARD_UNAVAILABLE ? M_PROPERTY_UNAVAILABLE
                                        : M_PROPERTY_ERROR;
  }
...
static int mp_property_clipboard(void *ctx, struct m_property *prop, int action,
                                 void *arg) {
  struct MPContext *mpctx = ctx;
  struct clipboard_access_params params = {
...
static const struct m_property mp_properties_base[] = {
    ...
    {"aid", mp_property_switch_track,
     .priv = (void *)(const int[]){0, STREAM_AUDIO}},
    ...
    {"vid", mp_property_switch_track,
     .priv = (void *)(const int[]){0, STREAM_VIDEO}},
    ...
    // Subs
    {"sid", mp_property_switch_track,
     .priv = (void *)(const int[]){0, STREAM_SUB}},
    ...
    {"secondary-sub-pos", mp_property_sub_pos, .priv = (void *)&(const int){1}},
    ...
    {"sub-start", mp_property_sub_start, .priv = (void *)&(const int){0}},
    {"secondary-sub-start", mp_property_sub_start,
     .priv = (void *)&(const int){1}},
    {"sub-end", mp_property_sub_end, .priv = (void *)&(const int){0}},
    {"secondary-sub-end", mp_property_sub_end, .priv = (void *)&(const int){1}},
    ...
    {"video-bitrate", mp_property_packet_bitrate,
     .priv = (void *)&(const int){STREAM_VIDEO}},
    {"audio-bitrate", mp_property_packet_bitrate,
     .priv = (void *)&(const int){STREAM_AUDIO}},
    {"sub-bitrate", mp_property_packet_bitrate,
     .priv = (void *)&(const int){STREAM_SUB}},
...
static const char *const *const mp_event_property_change[] = {
    ...
    E(MPV_EVENT_TICK, "time-pos", "audio-pts", "stream-pos", "avsync",
      ...
      "video-speed-correction", "vo-delayed-frame-count",
      "mistimed-frame-count", "vsync-ratio", "estimated-display-fps",
      "vsync-jitter", "sub-text", "secondary-sub-text", "audio-bitrate",
      ...
      "video-out-params", "video-dec-params", "video-params",
      "deinterlace-active", "video-target-params", "audio-fft"),
    E(MP_EVENT_DURATION_UPDATE, "duration"),
    E(MPV_EVENT_VIDEO_RECONFIG, "video-out-params", "video-params",
      "video-format", "video-codec", "video-bitrate", "dwidth", "dheight",
      "width", "height", "container-fps", "aspect", "aspect-name",
      "vo-configured", "current-vo", "video-dec-params", "osd-dimensions",
      "hwdec", "hwdec-current", "hwdec-interop", "window-id", "track-list",
      "current-tracks"),
    E(MPV_EVENT_AUDIO_RECONFIG, "audio-format", "audio-codec", "audio-bitrate",
      "samplerate", "channels", "audio", "volume", "volume-gain", "mute",
      "current-ao", "audio-codec-name", "audio-params", "track-list",
      "current-tracks", "audio-out-params", "volume-max", "volume-gain-min",
    ...
    E(MP_EVENT_CACHE_UPDATE, "demuxer-cache-duration", "demuxer-cache-idle",
      "paused-for-cache", "demuxer-cache-time", "cache-buffering-state",
      "cache-speed", "demuxer-cache-state"),
...
char *mp_property_expand_escaped_string(struct MPContext *mpctx,
                                        const char *str) {
  void *tmp = talloc_new(NULL);
  bstr strb = bstr0(str);
  bstr dst = {0};
...
static const struct property_osd_display {
  ...
  const char *msg;
    ...
    {"volume-gain", "Volume gain",
     .msg = "Volume gain: ${?volume-gain:${volume-gain} dB "
            "${?mute==yes:(Muted)}}${!volume-gain:${volume-gain}}",
    ...
    {"sub-visibility", .msg = "Subtitles ${!sub-visibility==yes:hidden}"
                              "${?sub-visibility==yes:visible${?sub==no: (but "
                              "no subtitles selected)}}"},
    {"secondary-sub-visibility",
     .msg = "Secondary Subtitles ${!secondary-sub-visibility==yes:hidden}"
            "${?secondary-sub-visibility==yes:visible${?secondary-sid==no: "
            "(but no secondary subtitles selected)}}"},
...
static void show_property_osd(MPContext *mpctx, const char *name,
  ...
  for (const struct property_osd_display *p = property_osd_display; p->name;
       p++) {
  ...
  if (disp.seek_msg || disp.seek_bar) {
    mpctx->add_osd_seek_info |= (osd_mode & MP_ON_OSD_MSG ? disp.seek_msg : 0) |
...
static void replace_overlay(struct MPContext *mpctx, int id,
                            struct overlay *new) {
  struct command_ctx *cmd = mpctx->command_ctx;
  mp_assert(id >= 0);
...
static struct track *find_track_with_url(struct MPContext *mpctx, int type,
  ...
  for (int n = 0; n < mpctx->num_tracks; n++) {
    ...
    if (track && track->type == type && track->is_external) {
      char *normalized =
          mp_normalize_user_path(NULL, mpctx->global, track->external_filename);
...
static bool check_property_scalable(char *property, struct MPContext *mpctx) {
  ...
  return prop.type == &m_option_type_float ||
         ...
         prop.type == &m_option_type_time || prop.type == &m_option_type_aspect;
}
...
static void show_property_status(struct mp_cmd_ctx *cmd, const char *name,
  ...
  if (r == M_PROPERTY_OK || r == M_PROPERTY_UNAVAILABLE) {
    ...
    set_osd_msg(mpctx, osdl, osd_duration, "Failed to set property '%s'", name);
    cmd->success = false;
  }
}

static void change_property_cmd(struct mp_cmd_ctx *cmd, const char *name,
                                int action, void *arg) {
...
static void cmd_cycle_values(void *p) {
  ...
  for (int n = first; n < cmd->num_args; n++) {
    ...
    if (m_option_parse(mpctx->log, &prop, bstr0(name), bstr0(cmd->args[n].v.s),
                       &val) < 0)
      continue;
...
static void on_cmd_list_sub_completion(struct mp_cmd_ctx *cmd) {
  ...
  if (list->current_valid &&
      mp_thread_id_equal(list->current_tid, mp_thread_current_id())) {
...
// current thread (e.g. from the main thread, or from within
// mp_dispatch_lock()). Completion means the command is finished, even if it
// errored or never ran. Keep in mind that calling this can execute further user
// command that can change arbitrary state (due to cmd_list).
...
void run_command(struct MPContext *mpctx, struct mp_cmd *cmd,
  ...
  *ctx = (struct mp_cmd_ctx){
  ...
  if (cmd->def->spawn_thread) {
    ...
    if (!mp_thread_pool_queue(mpctx->thread_pool, run_command_on_worker_thread,
                              ctx)) {
      mpctx->outstanding_async -= 1;
      ctx->success = false;
      mp_cmd_ctx_complete(ctx);
...
static void cmd_seek(void *p) {
  ...
  case 1:
    precision = MPSEEK_KEYFRAME;
    break;
  case 2:
    precision = MPSEEK_EXACT;
    break;
  }
  if (!mpctx->playback_initialized) {
    cmd->success = false;
  ...
  case 2: { // Absolute seek to a timestamp in seconds
    ...
    set_osd_function(mpctx, v > get_current_time(mpctx) ? OSD_FFW : OSD_REW);
    break;
  }
  case 3: { // Relative seek by percentage
    queue_seek(mpctx, MPSEEK_FACTOR,
               get_current_pos_ratio(mpctx, false) + v / 100.0, precision,
               MPSEEK_FLAG_DELAY);
    set_osd_function(mpctx, v > 0 ? OSD_FFW : OSD_REW);
    break;
  }
...
static void cmd_revert_seek(void *p) {
  ...
  if (cmd->args[0].v.i & 3) {
    ...
    queue_seek(mpctx, MPSEEK_ABSOLUTE, oldpts, MPSEEK_EXACT, MPSEEK_FLAG_DELAY);
...
static void cmd_set(void *p) {
  ...
  change_property_cmd(cmd, cmd->args[0].v.s, M_PROPERTY_SET_STRING,
                      cmd->args[1].v.s);
}
...
static void cmd_change_list(void *p) {
  ...
  m_option_free(&prop, &val);
  if (r < 0) {
    set_osd_msg(mpctx, osdl, osd_duration, "Failed setting option: '%s'", name);
    cmd->success = false;
    return;
  }
...
static void cmd_multiply(void *p) {
  ...
  change_property_cmd(cmd, cmd->args[0].v.s, M_PROPERTY_MULTIPLY,
                      &cmd->args[1].v.d);
}
...
static void cmd_playlist_next_prev_playlist(void *p) {
  ...
  if (!entry && mpctx->opts->loop_times != 1 && mpctx->playlist->current) {
    ...
    if (entry && entry->playlist_path &&
        ...
        strcmp(entry->playlist_path, mpctx->playlist->current->playlist_path) ==
            0)
      entry = NULL;
  ...
  if (sub && refpts != MP_NOPTS_VALUE) {
    ...
    if (sub_control(sub, SD_CTRL_SUB_STEP, a) > 0) {
      if (step) {
        ...
        m_config_notify_change_opt_ptr_notify(
            mpctx->mconfig, &mpctx->opts->subs_shared->sub_delay[track_ind]);
        show_property_osd(mpctx,
...
static void cmd_show_text(void *p) {
  ...
  set_osd_msg(mpctx, cmd->args[2].v.i,
              (cmd->args[1].v.i < 0 ? osd_duration : cmd->args[1].v.i), "%s",
              cmd->args[0].v.s);
}
...
static struct load_action get_load_action(struct MPContext *mpctx,
                                          int action_flag) {
  switch (action_flag) {
  case 0: // replace
...
static struct playlist_entry *get_insert_entry(struct MPContext *mpctx,
...
static void cmd_loadfile(void *p) {
  ...
  if (action.type == LOAD_TYPE_REPLACE ||
      (action.play && !mpctx->playlist->current)) {
...
static void cmd_loadlist(void *p) {
  ...
  char *path = mp_get_user_path(NULL, mpctx->global, filename);
  struct playlist *pl =
      playlist_parse_file(path, cmd->abort->cancel, mpctx->global);
  talloc_free(path);
...
static void cmd_playlist_remove(void *p) {
  ...
  struct playlist_entry *e =
      playlist_entry_from_index(mpctx->playlist, cmd->args[0].v.i);
  if (cmd->args[0].v.i < 0)
...
static void cmd_playlist_move(void *p) {
  ...
  struct playlist_entry *e1 =
      playlist_entry_from_index(mpctx->playlist, cmd->args[0].v.i);
  struct playlist_entry *e2 =
      playlist_entry_from_index(mpctx->playlist, cmd->args[1].v.i);
  if (!e1) {
    cmd->success = false;
    return;
  }
...
static void cmd_stop(void *p) {
  ...
  if (mpctx->opts->player_idle_mode < 2 && mpctx->opts->position_save_on_quit) {
    mp_write_watch_later_conf(mpctx);
  }
...
static void cmd_show_progress(void *p) {
  ...
  mpctx->add_osd_seek_info |= (cmd->msg_osd ? OSD_SEEK_INFO_TEXT : 0) |
...
static void cmd_track_reload(void *p) {
  ...
  if (t && t->is_external && t->external_filename) {
    ...
    mp_remove_track(mpctx, t);
    nt_num =
        mp_add_external_file(mpctx, filename, type, cmd->abort->cancel, flags);
    talloc_free(filename);
  }
  ...
  if (!nt->lang) {
    enum track_flags flags = 0;
    bstr lang =
        mp_guess_lang_from_filename(bstr0(nt->external_filename), NULL, &flags);
...
static void cmd_script_binding(void *p) {
  ...
  for (int i = 0; i < scale_units; i++) {
    event.num_args = 7;
    event.args = (const char *[7]){"key-binding",
                                   name,
                                   state,
                                   ...
                                   scale_s,
                                   cmd->args[1].v.s};
    if (mp_client_send_event_dup(mpctx, target, MPV_EVENT_CLIENT_MESSAGE,
                                 &event) < 0) {
...
static void cmd_script_message_to(void *p) {
  ...
  *event = (mpv_event_client_message){0};
  ...
  if (mp_client_send_event(mpctx, cmd->args[0].v.s, 0, MPV_EVENT_CLIENT_MESSAGE,
                           event) < 0) {
...
static void cmd_load_config_file(void *p) {
  ...
  int r = m_config_parse_config_file(mpctx->mconfig, mpctx->global, config_file,
                                     NULL, 0);
  talloc_free(ctx);
...
const struct mp_cmd_def mp_cmds[] = {
...

    {
        ...
        cmd_seek,
        {
            {"target", OPT_TIME(v.d)},
            {"flags",
             OPT_FLAGS(v.i, {"relative", 4 | 0}, {"-", 4 | 0},
                       {"absolute-percent", 4 | 1}, {"absolute", 4 | 2},
                       {"relative-percent", 4 | 3}, {"keyframes", 32 | 8},
                       {"exact", 32 | 16}),
             OPTDEF_INT(4 | 0)},
            // backwards compatibility only
            {"legacy",
             OPT_CHOICE(v.i, {"unused", 0}, {"default-precise", 0},
                        {"keyframes", 32 | 8}, {"exact", 32 | 16}),
    ...
    {"quit",
     cmd_quit,
     {{"code", OPT_INT(v.i), .flags = MP_CMD_OPT_ARG}},
     .priv = &(const bool){0}},
    {"quit-watch-later",
     cmd_quit,
     {{"code", OPT_INT(v.i), .flags = MP_CMD_OPT_ARG}},
    ...
    {"stop",
     cmd_stop,
     {{"flags", OPT_FLAGS(v.i, {"keep-playlist", 1}),
       .flags = MP_CMD_OPT_ARG}}},
    {
        "frame-step",
        cmd_frame_step,
        {
            ...
            {"flags", OPT_CHOICE(v.i, {"play", 0}, {"seek", 1}, {"mute", 2}),
             .flags = MP_CMD_OPT_ARG},
        },
        .allow_auto_repeat = true,
        .on_updown = true,
    ...
    {
        "playlist-next",
        cmd_playlist_next_prev,
        {
            {"flags", OPT_CHOICE(v.i, {"weak", 0}, {"force", 1}),
             .flags = MP_CMD_OPT_ARG},
        },
        .priv = &(const int){1},
    },
    ...
    {
        "playlist-shuffle",
        cmd_playlist_shuffle,
    },
    {
        "playlist-unshuffle",
        cmd_playlist_unshuffle,
    },
    {"sub-step",
     cmd_sub_step_seek,
     {
         {"skip", OPT_INT(v.i)},
         {"flags", OPT_CHOICE(v.i, {"primary", 0}, {"secondary", 1}),
          OPTDEF_INT(0)},
     },
     .allow_auto_repeat = true,
...

    {
        ...
        cmd_track_add,
        {
            {"url", OPT_STRING(v.s)},
            {"flags",
             OPT_FLAGS(v.i, {"select", 0}, {"auto", 1}, {"cached", 2},
                       {"hearing-impaired", TRACK_HEARING_IMPAIRED},
                       {"visual-impaired", TRACK_VISUAL_IMPAIRED},
                       {"forced", TRACK_FORCED}, {"default", TRACK_DEFAULT}),
             .flags = MP_CMD_OPT_ARG},
...

    {
        ...
        cmd_rescan_external_files,
        {
            {"flags", OPT_CHOICE(v.i, {"keep-selection", 1}, {"reselect", 0}),
             .flags = MP_CMD_OPT_ARG},
        },
        .spawn_thread = true,
        .can_abort = true,
...

    {
        ...
        {
            {"flags",
             OPT_FLAGS(v.i, {"video", 0}, {"scaled", 1}, {"subtitles", 2},
                       {"osd", 4}, {"each-frame", 8}, {"-", 0},
                       {"window", 1 | 2 | 4}),
             OPTDEF_INT(2)},
            // backwards compatibility
            {"legacy",
             OPT_CHOICE(v.i, {"unused", 0}, {"single", 0}, {"each-frame", 8}),
             .flags = MP_CMD_OPT_ARG},
        },
        .spawn_thread = true,
    },
    {
        ...
        cmd_screenshot_to_file,
        {
            ...
            {"flags",
             OPT_CHOICE(v.i, {"video", 0}, {"scaled", 1}, {"subtitles", 2},
                        {"osd", 4}, {"window", 1 | 2 | 4}),
             OPTDEF_INT(2)},
        },
        .spawn_thread = true,
    },
    {
        ...
        cmd_screenshot_raw,
        {
            {"flags",
             ...
             OPTDEF_INT(2)},
            {"format",
             OPT_CHOICE(v.i, {"bgr0", 0}, {"bgra", 1}, {"rgba", 2},
                        {"rgba64", 3}),
             OPTDEF_INT(0)},
        },
    },
    {
        ...
        cmd_loadfile,
        {
            {"url", OPT_STRING(v.s)},
            {"flags",
             OPT_CHOICE(v.i, {"replace", 0}, {"append", 1}, {"append-play", 2},
                        {"insert-next", 3}, {"insert-next-play", 4},
                        {"insert-at", 5}, {"insert-at-play", 6}),
             .flags = MP_CMD_OPT_ARG},
    ...
    {"playlist-remove",
     cmd_playlist_remove,
     {
         {"index", OPT_CHOICE(v.i, {"current", -1}), M_RANGE(0, INT_MAX)},
     }},
    {"playlist-move",
     cmd_playlist_move,
     {
    ...
    {"multiply",
     cmd_multiply,
     {{"name", OPT_STRING(v.s)}, {"value", OPT_DOUBLE(v.d)}},
     .allow_auto_repeat = true},
    ...
    {"enable-section",
     cmd_enable_input_section,
     {
         {"name", OPT_STRING(v.s)},
         {"flags",
          OPT_FLAGS(v.i, {"default", 0}, {"exclusive", MP_INPUT_EXCLUSIVE},
                    {"allow-hide-cursor", MP_INPUT_ALLOW_HIDE_CURSOR},
                    {"allow-vo-dragging", MP_INPUT_ALLOW_VO_DRAGGING}),
          .flags = MP_CMD_OPT_ARG},
     }},
    {"disable-section", cmd_disable_input_section, {{"name", OPT_STRING(v.s)}}},
    {
        "define-section",
        cmd_define_input_section,
        {
            {"name", OPT_STRING(v.s)},
    ...
    {"script-binding",
     ...
     {
         {"name", OPT_STRING(v.s)},
         {"arg", OPT_STRING(v.s), OPTDEF_STR(""), .flags = MP_CMD_OPT_ARG},
     },
     .allow_auto_repeat = true,
     .on_updown = true,
     .scalable = true},
    ...
    {"script-message",
     ...
     .vararg = true},
    {"script-message-to",
     cmd_script_message_to,
     {{"target", OPT_STRING(v.s)}, {"args", OPT_STRING(v.s)}},
     .vararg = true},
    ...
    {"mouse",
     ...
     {{"x", OPT_INT(v.i)},
      ...
      {"mode", OPT_CHOICE(v.i, {"single", 0}, {"double", 1}),
       .flags = MP_CMD_OPT_ARG}}},
    {"keybind",
     cmd_key_bind,
     {{"name", OPT_STRING(v.s)},
    ...
    {"keypress",
     cmd_key,
     {{"name", OPT_STRING(v.s)}, {"scale", OPT_DOUBLE(v.d), OPTDEF_DOUBLE(1)}},
     .priv = &(const int){0}},
    {"keydown",
     cmd_key,
     {{"name", OPT_STRING(v.s)}},
    ...
    {"keyup",
     cmd_key,
     {{"name", OPT_STRING(v.s), .flags = MP_CMD_OPT_ARG}},
...

    {
        "dump-cache",
        cmd_dump_cache,
        {{"start", OPT_TIME(v.d), .flags = M_OPT_ALLOW_NO},
         {"end", OPT_TIME(v.d), .flags = M_OPT_ALLOW_NO},
         {"filename", OPT_STRING(v.s)}},
        .exec_async = true,
        .can_abort = true,
    },
  ...
  *ctx = (struct command_ctx){
  ...
  for (int n = 0; n < num_opts; n++) {
    ...
    if (co->opt->type == &m_option_type_alias) {
      ...
      const char *alias = m_config_shadow_get_alias_from_opt(
          mpctx->mconfig->shadow, co->opt_id, buf, sizeof(buf));
      prop.priv = talloc_strdup(ctx, alias);

      prop.call = co->opt->deprecation_message ? mp_property_deprecated_alias
                                               : mp_property_alias;
      ...
      while (co2 && co2->opt->type == &m_option_type_alias) {
        const char *co2_alias = m_config_shadow_get_alias_from_opt(
            mpctx->mconfig->shadow, co2->opt_id, buf, sizeof(buf));
    ...
    if (bsearch(&prop.name, prop_names, num_base, sizeof(const char *),
                str_compare))
      continue;
...
void run_command_opts(struct MPContext *mpctx) {
  ...
  for (int i = 0; opts->input_commands[i]; i++) {
    struct mp_cmd *cmd = mp_input_parse_cmd(
        mpctx->input, bstr0(opts->input_commands[i]), "the command line");
    if (cmd) {
      cmd->coalesce = true;
...
static void update_track_switch(struct MPContext *mpctx, int order, int type) {
  ...
  mp_switch_track_n(mpctx, order, type, track,
                    (tid == -1) ? 0 : FLAG_MARK_SELECTION);
  print_track_list(mpctx, "Track switched:");
  mp_wakeup_core(mpctx);
}

void mp_option_change_callback(void *ctx, struct m_config_option *co,
  ...
  for (int i = 0; i < mpctx->num_option_callbacks; i++) {
    if ((mpctx->option_callbacks[i].co &&
         opt_ptr == mpctx->option_callbacks[i].co->data) ||
  ...
  if (!drop) {
    struct mp_option_callback callback = {.co = co, .flags = flags};
    MP_TARRAY_APPEND(mpctx, mpctx->option_callbacks,
                     mpctx->num_option_callbacks, callback);
  }
}

void mp_option_run_callback(struct MPContext *mpctx,
                            struct mp_option_callback *callback) {
  struct MPOpts *opts = mpctx->opts;
  ...
  if (opt_ptr == &opts->play_dir) {
    if (mpctx->play_dir != opts->play_dir) {
      ...
      queue_seek(mpctx, MPSEEK_ABSOLUTE, get_current_time(mpctx), MPSEEK_EXACT,
                 0);
    }
  }
