...
void update_screensaver_state(struct MPContext *mpctx) {
  ...
  bool saver_state =
      (!mpctx->playback_active || !mpctx->opts->stop_screensaver) &&
      mpctx->opts->stop_screensaver != 2;
  vo_control_async(
      mpctx->video_out,
      saver_state ? VOCTRL_RESTORE_SCREENSAVER : VOCTRL_KILL_SCREENSAVER, NULL);
}
...
double get_current_pos_ratio(struct MPContext *mpctx, bool use_range) {
  ...
  if (use_range) {
    if (mpctx->opts->play_frames > 0)
      ret = MPMAX(ret,
                  1.0 - mpctx->max_frames / (double)mpctx->opts->play_frames);
  }
  return ret;
}
...
int get_current_chapter(struct MPContext *mpctx) {
  ...
  for (i = 0; i < mpctx->num_chapters; i++)
    if (current_pts < mpctx->chapters[i].pts)
      break;
  return mpctx->last_chapter_flag ? mpctx->last_chapter_seek
                                  : MPMAX(mpctx->last_chapter_seek, i - 1);
}
...
int get_chapter_count(struct MPContext *mpctx) { return mpctx->num_chapters; }
...
static void handle_update_cache(struct MPContext *mpctx) {
  ...
  if (!mpctx->restart_complete) {
    ...
    use_pause_on_low_cache &=
        opts->cache_pause_initial && (mpctx->video_status == STATUS_READY ||
  ...
  if (mpctx->cache_buffer != cache_buffer) {
    if ((mpctx->cache_buffer == 100) != (cache_buffer == 100)) {
      if (cache_buffer < 100) {
        MP_VERBOSE(mpctx,
                   "Enter buffering (buffer went from %d%% -> %d%%) [%fs].\n",
      ...
      MP_VERBOSE(mpctx,
                 "Still buffering (buffer went from %d%% -> %d%%) [%fs].\n",
                 mpctx->cache_buffer, cache_buffer, s.ts_info.duration);
    }
...
static void handle_eof(struct MPContext *mpctx) {
  ...
  bool prevent_eof = mpctx->paused && mpctx->video_out &&
